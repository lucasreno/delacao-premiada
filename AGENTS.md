...
Proibido o uso de travessão (—) em qualquer lugar do projeto (código, comentários, UI, docs, ADRs) - use vírgula, ponto ou hífen simples (-). O prompt do `classifier.py` também instrui a IA a não usar travessão nas Descrições geradas.
